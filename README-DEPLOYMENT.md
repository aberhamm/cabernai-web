...
**Now:** Uses **only ONE comprehensive `.env` file** that automatically loads into all containers
...
| **GitHub Actions**        | GitHub Secrets create the single comprehensive `.env` file automatically on the server |
...
**✨ Important: Create GitHub Secrets with the EXACT same names as your .env variables for consistency!**
...
DOMAIN_NAME             # Your domain (for verification checks)

# URLs (generated by env-generator.sh)
UI_PUBLIC_URL
STRAPI_PUBLIC_URL
NEXT_PUBLIC_APP_PUBLIC_URL
NEXT_PUBLIC_STRAPI_URL
NEXTAUTH_URL
PUBLIC_URL              # For Strapi
APP_URL                 # For Strapi

# Database
SUPABASE_DATABASE_URL
DATABASE_URL            # Usually same value as SUPABASE_DATABASE_URL
...
| 2b   | **GitHub Actions:** Copy environment values to GitHub Secrets | ☁️ GitHub        | GitHub creates comprehensive `.env` file on server during deployment |
...
```bash
# Domain Configuration (for both frontend and backend)
